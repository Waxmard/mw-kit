...
- **Some/all targets present** → this is the drift question. Read the page's canonical `## Config` block and the present target file(s), then classify match vs drift.
  - **`agent-instructions` is content-checked, not just presence-checked.** This page has no canonical `## Config` snippet, so "both files present" is *not* automatically ✅. Read `AGENTS.md` and classify ⚠️ drift when: (a) `CLAUDE.md` exists but isn't a symlink to `AGENTS.md` (two real files that can diverge), or (b) the body contains **any** specific-agent reference — `Claude`, `Claude Code`, `Gemini`, a `# CLAUDE.md` heading, any product name. The rule is zero named agents in the prose; always "the agent" / "AI agents". A `../CLAUDE.md`-style pointer to a sibling that's genuinely named that gets reworded to drop the name (not left as-is). The only place an agent filename is allowed is a symlink *alias* (`CLAUDE.md` → `AGENTS.md`), which is structure, not body.
...
3. Show the proposed change as a diff and **confirm before writing**. Then apply with Edit (fragment merge) or Write (new whole file).
   - **Symlink targets (`agent-instructions`).** This page's "apply" is filesystem plumbing, not a config merge: `AGENTS.md` is the real file and `CLAUDE.md` / `GEMINI.md` are symlinks to it. If the repo has a real `CLAUDE.md` and no `AGENTS.md`, the fix is `mv CLAUDE.md AGENTS.md` then `ln -s AGENTS.md CLAUDE.md` (Bash, confirm first; plain `mv`, not `git mv` — staging stays the user's job) — never Write a second copy. If `AGENTS.md` already exists and `CLAUDE.md` is a divergent real file, surface the conflict instead of clobbering; let the user pick which content wins, then symlink. Add a `GEMINI.md` symlink only if the user uses Gemini CLI. A repo that *generates* both files from a template is ⚠️ drift, not satisfied: propose collapsing it to the symlink and deleting whatever existed only to duplicate the guide — the template, the build script, its `make` targets, the CI staleness gate, the pre-commit hook. Scope that carefully before proposing it: a generator that also renders genuinely different documents (a README from partials) stays, minus the agent guides; one whose remaining outputs are 1:1 copies goes entirely.
     - **Scrub the body to agent-agnostic after the move.** The rename alone isn't the fix — the page's contract is neutral content, so once `AGENTS.md` is the shared file, rewrite Claude-specific phrasing in it (`Claude Code` / "Claude" → "the agent" / "AI agents"; drop Claude-only asides) and show that edit as part of the diff. Keep a named-agent line only where the instruction genuinely applies to just that one. Skipping this leaves every other agent reading "Claude"-flavored guidance.
